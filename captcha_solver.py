...
WHAT THIS MODULE IS FOR ON MERCOR
---------------------------------
**Nothing, as measured — and the honest version of that sentence is narrow.**

On 2026-09-18, from a Hetzner datacentre address (AS24940, Helsinki),
Mercor served every route this scraper reads. Eighteen candidate challenge
markers were counted across seven captures (`/explore`, `/careers`, the
homepage, two detail pages, two 404s) and every one was zero. Identical 200s
came back from `curl`, from `python-requests` and from an EMPTY
User-Agent, and twelve rapid sequential detail fetches were all 200 at about
0.4s each. No challenge has ever been rendered to this scraper.
...
and on this site that distinction has teeth:
  ...
  * **The MARKETPLACE host runs invisible reCAPTCHA Enterprise; the
    ...
    It arrives in a lazily-loaded bundle. So §18's advice to "grep a good
    page for the site's own captcha config" — a `*_SITE_KEY`, a
    `<captcha-*>` element, a `data-sitekey` — returns nothing on this site,
    and a reader working from `--dump-html` alone would conclude no captcha
    is configured. Only a real browser reveals it. That is a new shape of
    the §18 lesson and the reason it is recorded here rather than left to
    be rediscovered.

  * **Cloudflare fronts the site and has never challenged it.** Every page
    carries `/cdn-cgi/challenge-platform` — counted once on all seven
    captures, the 404s included — which is Cloudflare's ordinary bot
    management script and NOT a challenge. It was briefly in this repo's
    marker set and made the 404 page classify as `blocked`; it is gone. See
    `product_parser.BOT_CHALLENGE_MARKERS`.

So this module is insurance on Mercor rather than a route in use. The
...
They are evidence about the SOLVER, not about Mercor, and are labelled as
